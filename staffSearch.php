...
<form id="person-search" class="general" method="get">
    ...
    <input type="text" id="id" name="id" value="<?php if (isset($id)) echo htmlspecialchars($_GET['id']) ?>" placeholder="Enter the staff's username/ID">
    ...
    if (isset($_GET['name'])) {
        ...
        $staff = array_merge($admins, $superadmins);
        ...
        if (count($staff) > 0) {
            ...
            foreach ($staff as $person) {
                $printType = $person->get_type();
                if ($printType == "admin") {
                    $printType = "Admin";
                } else if ($printType == "superadmin") {
                ...
                <tr>
                    ...
                    <td>' . $printType . '</td>
                    <td><a href="modifyUserRole.php?id=' . $person->get_id() . '">Modify Account Type</a></td>
                </tr>';
            }
